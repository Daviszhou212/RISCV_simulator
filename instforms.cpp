...
IFormInst::encodeSlli(unsigned rd, unsigned rs1, unsigned shamt)
{
  /* INSERT YOUR CODE HERE */
  if (shamt > (1 << 5))
    return false;  // Must fit in 5 bits
  fields2.top7 = 0;
  fields2.opcode = 0x13;
  fields2.funct3 = 1;
...
#pragma GCC diagnostic ignored "-Wconversion"
  ...
  bits.imm12 = 0;  
  bits.imm11 = (imm & 0x800)>>11;
  bits.imm4_1 = (imm & 0x1e)>>1;
  bits.imm10_5 = (imm & 0x7e0)>>5;
  #pragma GCC diagnostic pop
  return true;
}
...
{
  /* INSERT YOUR CODE HERE */
  if (immed > (1 << 20) or -immed < -(1 << 20))
    return false;  // Must fit in 21 bits  
  bits.opcode = 0x37;
  bits.rd = rdv & 0x1f;
  #pragma GCC diagnostic push
  #pragma GCC diagnostic ignored "-Wconversion"
  bits.imm = immed & 0x0fffff;
  #pragma GCC diagnostic pop
  return true;
}
...
JFormInst::encodeJal(uint32_t rdv, int offset)
{
  ...
  bits.imm19_12 = (offset & 0x0ff000)>>12;  
  bits.imm10_1 = (offset & 0x7fe)>>1;
  bits.imm11 = (offset & 0x800)>>11;
  #pragma GCC diagnostic pop
  return true;
}
